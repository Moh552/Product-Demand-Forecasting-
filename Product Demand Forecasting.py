...
def dei(file_path, column_name):
    ...
    def pt(x_data, y_pred):
        plt.figure(figsize=(8, 5))
        plt.scatter(range(len(y_pred)), y_pred, c="red", marker='D', edgecolors='k', s=50)
        plt.title("Predicted Data")
...
def test(): 
    ...
    icon = Image.open("C:\\Users\\Asus\\Documents\\learn\\io.jpg")
...
def train():
    fram = Toplevel()
    fram.title("Load Trained Model")
    fram.geometry("500x500")
    fram.configure(bg='#FEF3E2')

    text_size = ("Times New Roman", 14, "italic")

    label = Label(fram, text="Load a Trained Model", font=text_size, bg="#FA4032", fg="white")
    label.pack(pady=10)

    model_entry = Entry(fram, width=50, font=("Times New Roman", 12, "italic"), bg="white", relief=SOLID)
    ...
    btn_browse = Button(fram, text="Select Model", command=select_model, bg="blue", fg="white", font=("Arial", 12, "bold"))
    ...
    btn_load = Button(fram, text="Load Model", command=load_model, bg="green", fg="white", font=("Arial", 12, "bold"))
    btn_load.pack(pady=10)

    fram.mainloop()

# Function to initialize the main GUI window
def main():
    root = Tk()  
    root.title("gj")
    root.geometry('300x200')

    icon_image = Image.open("C:\\Users\\Asus\\Documents\\learn\\io.jpg")
    icon = ImageTk.PhotoImage(icon_image)
    root.iconphoto(True, icon)

    root.configure(bg='#FEF3E2')
    le1 = Label(root, text="test window", font=("Times New Roman", 12, "italic"), bg="#FA812F")
    le1.grid(row=1, column=1, padx=(80, 10), pady=10)

    btn_test = Button(root, text="open test window", command=test, bg="blue", fg="white")
    btn_test.grid(row=2, column=1, padx=(80, 10), pady=10)

    le2 = Label(root, text="train window", font=("Times New Roman", 12, "italic"), bg="#FA812F")
    le2.grid(row=3, column=1, padx=(80, 10), pady=10)

    btn_train = Button(root, text="open train window", command=train, bg="blue", fg="white")
